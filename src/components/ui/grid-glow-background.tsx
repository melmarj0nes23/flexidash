...
export const GridGlowBackground: React.FC<GridGlowBackgroundProps> = ({
  ...
  gridSize = 50,
  glowColors = ["#19c985", "#14a16b", "#19c985"],
  glowCount = 10,
}) => {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden" style={{ backgroundColor }}>
      <canvas
        ref={canvasRef}
        className="absolute inset-0 z-0 h-full w-full opacity-50 pointer-events-none"
      />
      <div className="relative z-10 flex min-h-screen items-center justify-center pointer-events-auto w-full">
        {children}
      </div>
    </div>
  );
