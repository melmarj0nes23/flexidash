...
export default async function EntryPage({
  ...
  const [transactions, totalCount] = await Promise.all([
    getTransactions(userId, { limit, offset, searchQuery, type: 'expense' }),
    getTransactionCount(userId, { searchQuery, type: 'expense' })
  ]);
  ...
  if (editId && !initialData) {
     const allTxForEdit = await getTransactions(userId, { type: 'expense' }); 
    ...
    <div className="space-y-6">
      ...
      <section className="bg-white dark:bg-[#1b1d22] rounded-lg shadow-sm border border-gray-200 dark:border-[#2a2c33] overflow-hidden mt-6">
        <div className="p-4 md:p-6 border-b border-gray-100 dark:border-[#2a2c33] flex flex-col md:flex-row md:items-center justify-between gap-4">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Expense History</h3>
          <SearchBar />
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-[#121417] border-b border-gray-200 dark:border-[#2a2c33]">
                ...
                <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">Category / Vendor</th>
              ...
              {transactions.length === 0 ? (
                <tr>
                  <td colSpan={4 + normalizedFields.length} className="p-8 text-center text-gray-500 dark:text-gray-400">
                    No expenses logged yet. Start logging above!
                  </td>
                </tr>
              ) : (
                transactions.map((t: any) => {
                  ...
                  return (
                    <tr key={t.id} className="border-b border-gray-100 dark:border-[#2a2c33] last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-[#121417] transition-colors">
                      ...
                      <td className="p-4 text-sm font-bold text-red-500 dark:text-red-500 text-right whitespace-nowrap">{currencySymbol}{Math.abs(t.price_charged).toFixed(2)}</td>
